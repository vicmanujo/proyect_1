...
// ==========================================
// Arrancamos en true para que no parpadee feo al cargar
...
const verificarPermisos = async () => {
  try {
    ...
    // Leemos los permisos reales del perfil desde el backend
    ...
    if (data.success) {
      ...
      if (moduloEncontrado) {
        // En los estáticos también leemos el "consulta" para ocultar toda la tabla si no tiene acceso
        permisosModuloActual.value = {
          consulta: true, // Si nos lo mandó el backend, es porque sí puede consultar
        ...
        // Si no tiene permisos, bloqueamos hasta la vista de la tabla
...
const registros = ref([
  ...
  { id: 4, strCodigo: 'PRY-100', strProducto: 'App Móvil iOS', numStock: 80, estado: 'En Progreso' },
  { id: 5, strCodigo: 'PRY-101', strProducto: 'Migración Nube', numStock: 100, estado: 'Completado' }
])
...
// 🟢 DISPARAMOS LA VERIFICACIÓN AL ENTRAR
onMounted(() => {
  verificarPermisos()
})
